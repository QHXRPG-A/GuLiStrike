...
| `Archive/` | 每次开发的变更记录：细节与结果 | `YYYYMMDD-解决了什么事.md` | 允许新增或合并；合并完成后必须删除被替代的旧归档 |
...
2. 可以写入新的 `Archive/YYYYMMDD-解决了什么事.md`（模板 C），也可以合并相关旧归档。合并时创建覆盖全部原始事实、复现信息与验证结论的新合并归档；确认新归档完整、相关引用已迁移后，必须删除被替代的旧归档。
...
- **新增测试必须先询问用户并取得明确许可。** 未经许可，不得创建测试文件、追加测试用例、扩充测试角色/Gate、扩大测试矩阵或把当前验证延伸到相邻模块。
- 只允许提议和添加与当前已确认需求直接相关的测试；提议时必须说明对应需求、拟新增的测试内容及涉及文件，询问用户是否添加。
- 用户许可只覆盖当次明确说明的测试范围，不得据此继续扩散。发现额外风险时先记录为待确认项，不自行增加测试。
- 运行已经存在且属于当前需求既定验收范围的测试，不视为“新增测试”；但修改或扩充这些测试仍须先询问用户。
...
- 归档允许新增或合并；合并时必须先保证内容完整并迁移引用，随后删除被替代旧归档。玩法文档只改不增（原地演进）；索引每次归档或合并后更新。
- 未经用户明确许可不得新增或扩充测试；获准后也只能添加与当前需求直接相关的测试内容。
