...
代码与功能开发的验证边界遵循 [SKILL.md 的“开发验证与场景交付”](../SKILL.md#开发验证与场景交付)：测试场景作为此类开发最后一步，保存并核对相关实体数据后即可总结交付，无需截图或等待玩家反馈。代码静态检查和场景实体数据核对不等于玩家效果验收；`status: verification`、`verification: partial` 可表示本轮已交付、后续待玩家验证，不要求助手继续等待或追加测试。

美术资产制作按对应制作阶段、视觉验证证据和用户审核结果更新状态，不套用上述代码开发的收尾条件。混合任务分别记录代码与美术验证边界，不用代码检查或场景实体数据核对代替美术审核通过。
