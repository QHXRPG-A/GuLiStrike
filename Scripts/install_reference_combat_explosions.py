...
def run():
    ...
    keep=[line for line in old.splitlines() if not any(line.startswith(s) for s in ['+Explosions=','+WingmanExplosions=','GroundExplosionScaleParameter=','WingmanExplosionScaleParameter=','; Wingman deaths use'])]
    keep+=['; Approved reference explosions: range is passed once; component scale stays one.',
           ...
           '+Explosions='+paths['ground']+'.'+paths['ground'].rsplit('/',1)[1],
           '+WingmanExplosions='+paths['air']+'.'+paths['air'].rsplit('/',1)[1]]
    ...
    c.set_editor_property('Explosions',[unreal.load_asset(paths['ground'])])
    c.set_editor_property('WingmanExplosions',[unreal.load_asset(paths['air'])])
    ...
    v=unreal.GuLiEffectVisualVariant();v.set_editor_property('system',unreal.load_asset(paths['bomb']))
    v.set_editor_property('scale',5.);v.set_editor_property('scale_parameter_name','User.Area_Scale')
    ...
    REPORT.update(success=True,after=after,settings={n:str(c.get_editor_property(n)) for n in ['Explosions','WingmanExplosions','GroundExplosionScaleParameter','WingmanExplosionScaleParameter']},unchanged_gameplay=guard,paths=paths)
