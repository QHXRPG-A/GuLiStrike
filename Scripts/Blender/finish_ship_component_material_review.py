...
def run(key):
    ...
    if arm:
        ...
        # This Blender build needs an external encoder. Do not depend on a
        # removed session temp directory; use configured or installed FFmpeg.
        ffmpeg=os.environ.get('GULI_FFMPEG_EXE') or shutil.which('ffmpeg')
        if not ffmpeg:
            try:
                import imageio_ffmpeg
                ffmpeg=imageio_ffmpeg.get_ffmpeg_exe()
            except ImportError as exc:
                raise RuntimeError('Regenerating motion videos requires FFmpeg on PATH or GULI_FFMPEG_EXE pointing to ffmpeg.exe.') from exc
        if not Path(ffmpeg).is_file():raise RuntimeError('FFmpeg executable is unavailable: '+str(ffmpeg))
        ...
        result=subprocess.run([ffmpeg,'-y','-framerate','24','-start_number','1',
