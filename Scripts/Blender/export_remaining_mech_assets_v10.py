"""Export the five remaining produced assets without changing their authored style.
...
OUT=ROOT/'UE_AllAssets_v10';(OUT/'FBX').mkdir(parents=True,exist_ok=True);(OUT/'Textures').mkdir(exist_ok=True)
...
frozen={}
for key in ('Mecha_01','Mecha_02','FireWeapon_01','MissileWeapon_01','Missile_01'):
...
def surface_source(mat,role):
    if role=='BaseColor':
        cel=mat.node_tree.nodes.get('Paint_x_Three_Tones')
        p=next(n for n in mat.node_tree.nodes if n.type=='BSDF_PRINCIPLED')
        return cel.inputs[1] if cel else p.inputs['Base Color'],1.
    if role in ('Roughness','Metallic'):
        return next(n for n in mat.node_tree.nodes if n.type=='BSDF_PRINCIPLED').inputs[role],1.
    if role=='LineMask':
...
def bake(o,name,role,size,per_slot=False):
    ...
    for i,mat in enumerate(o.data.materials):
        ...
        if i==0 or per_slot:
            ...
            im.colorspace_settings.name='Non-Color' if role in ('LineMask','Roughness','Metallic') else 'sRGB'
...
parts=[('Mecha_01','pbr'),('Mecha_02','pbr'),('FireWeapon_01','ink_cel'),('MissileWeapon_01','ink_cel'),('Missile_01','pbr')]
for name,style in parts:
    collection=bpy.data.collections['WORK_'+name]
    bodies,outlines=copy_meshes(collection,None,Matrix.Identity(4))
    ...
    roles=('BaseColor','LineMask','Emissive') if style=='ink_cel' else ('BaseColor','Roughness','Metallic','Emissive')
    maps={role:bake(body,name,role,2048) for role in roles}
    for uv in list(body.data.uv_layers):
        if uv.name!='GameAtlas':body.data.uv_layers.remove(uv)
    body.data.materials.clear();body.data.materials.append(flat_material('Export_'+name+'_Surface'))
    ...
    if outlines:
        ...
        edge.data.uv_layers.new(name='GameAtlas')
        mesh=join([body,edge],'Styled_'+name,normalize=False)
    rig=build_rig(name) if snapshot['meshes'][name].get('bones') else None
    if rig:
        assert all(v.groups for v in mesh.data.vertices),name
        mod=mesh.modifiers.new('Original_UE_Skeleton','ARMATURE');mod.object=rig
    select([mesh]+([rig] if rig else []),mesh)
    ...
    report['parts'][name]={'fbx':str(path),'fbx_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'body_triangles':body_tri,'outline_triangles':ink_tri,
        'bones':len(rig.data.bones) if rig else 0,'body_material_slots':1,'ink_slot':1 if ink_tri else None,'textures':maps,'style':style,
        'original_path':snapshot['meshes'][name]['path'],
    ...
    print('ASSET_EXPORTED',name,flush=True)
report['success']=True
(OUT/'export_report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
print('ALL_REMAINING_ASSETS_EXPORTED',flush=True)
