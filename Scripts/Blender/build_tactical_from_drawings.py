...
def lathe(name,center,profile,colors,axis='Y',sides=16,motion='Body'):
    ...
    for along,radius in profile:
        for i in range(sides):
            ...
            q=(radius*math.cos(angle),along,radius*math.sin(angle)) if axis=='Y' else (along,radius*math.cos(angle),radius*math.sin(angle)) if axis=='X' else (radius*math.cos(angle),radius*math.sin(angle),along)
...
def shoulder_crown(x,z):
    """The approved shoulder is a formed convex shell, not a flat extrusion.

    A single broad quadratic crown gives a regular, dent-free surface.  The
    long axis follows the back-to-front slope; both shoulders share this datum.
    """
    dx,dz=x+.70,z-2.46
    along=.84*dx-.54*dz;across=.54*dx+.84*dz
    return .52*(1-(along/2.55)**2-(across/1.50)**2)

def crowned_shoulder(name,profile):
    """One watertight formed shell, with paint/seam boundaries in the topology.

    All colored regions share the same curved surface; no flat decals or
    overlapping white plates can float above or intersect the orange armor.
    """
    from mathutils.geometry import delaunay_2d_cdt
    center=Vector((-.82,2.58))
    def scaled(poly,factor):return [tuple(center+(Vector(p)-center)*factor) for p in poly]
    face_outline=scaled(profile,.95)
    inset=[(-2.18,2.35),(-1.64,2.81),(-.95,2.62),(-.25,1.43),(-.77,1.37),(-1.96,1.93)]
    inset_center=sum((Vector(p) for p in inset),Vector((0,0)))/len(inset)
    seam=[tuple(inset_center+(Vector(p)-inset_center)*1.016) for p in inset]
    stripe=[(-1.99,3.57),(-1.43,3.19),(-1.17,2.56),(-1.65,2.40),(-1.91,3.15)]
    top=[(-2.55,3.866),(-1.98,3.687),(-1.44,3.392),(-1.62,3.19),(-2.45,3.50)]
    c=Vector((-1.37,2.59));d=Vector((math.cos(math.radians(-68)),math.sin(math.radians(-68))))*.145;q=Vector((-d.y,d.x))*(.020/.145)
    slot=[tuple(c-d-q),tuple(c+d-q),tuple(c+d+q),tuple(c-d+q)]
    coatings=[(top,1),(stripe,1),(seam,3),(inset,1),(slot,3)]
    def inside(point,poly):
        x,y=point;hit=False
        for p0,p1 in zip(poly,poly[1:]+poly[:1]):
            if (p0[1]>y)!=(p1[1]>y) and x<(p1[0]-p0[0])*(y-p0[1])/(p1[1]-p0[1])+p0[0]:hit=not hit
        return hit
    points=[];edges=[];outline_loop=[]
    # Constrained boundaries are sampled too, so the silhouette and paint have
    # the same curvature resolution as the broad orange surface.
    for poly in [face_outline]+[p for p,_ in coatings]:
        loop=[]
        for p0,p1 in zip(poly,poly[1:]+poly[:1]):
            p0,p1=Vector(p0),Vector(p1);steps=max(1,math.ceil((p1-p0).length/.32))
            for k in range(steps):loop.append(len(points));points.append(p0.lerp(p1,k/steps))
        edges.extend((loop[i],loop[(i+1)%len(loop)]) for i in range(len(loop)))
        if not outline_loop:outline_loop=loop[:]
    for ix in range(14):
        for iz in range(10):
            p=Vector((-2.70+ix*.29,1.25+iz*.29))
            if inside(p,face_outline):points.append(p)
    if sum(points[a].x*points[b].y-points[b].x*points[a].y for a,b in zip(outline_loop,outline_loop[1:]+outline_loop[:1]))<0:outline_loop.reverse()
    vv,_,ff,_,_,orig_faces=delaunay_2d_cdt(points,edges,[outline_loop],0,2e-5,True)
    front=[tuple(f) for f,ids in zip(ff,orig_faces) if 0 in ids]
    used=sorted({i for f in front for i in f});remap={v:i for i,v in enumerate(used)}
    xy=[vv[i] for i in used];front=[tuple(remap[i] for i in f) for f in front]
    verts=[(p.x,1.58+shoulder_crown(p.x,p.y),p.y) for p in xy];faces=list(front);colors=[]
    for f in front:
        p=sum((xy[i] for i in f),Vector((0,0)))/len(f);ci=0
        for region,color in coatings:
            if inside(p,region):ci=color
        colors.append(ci)
    counts={}
    for f in front:
        for a,b in zip(f,f[1:]+f[:1]):
            edge=tuple(sorted((a,b)));counts[edge]=counts.get(edge,0)+1
    boundary=[e for e,n in counts.items() if n==1];rim={};back={};wrap={}
    for i in {v for e in boundary for v in e}:
        p=center+(xy[i]-center)/.95
        rim[i]=len(verts);verts.append((p.x,1.50+shoulder_crown(p.x,p.y),p.y))
        back[i]=len(verts);verts.append((p.x,1.03,p.y))
        wrap[i]=len(verts);verts.append(tuple(Vector(verts[rim[i]]).lerp(Vector(verts[back[i]]),.54)))
    for a,b in boundary:
        faces.append((a,b,rim[b],rim[a]));colors.append(0)
        pa,pb=Vector(verts[rim[a]]),Vector(verts[rim[b]]);mid=(pa+pb)*.5
        # The ivory band wraps onto the inward slope. All neighboring strips
        # share the wrap vertices, so paint boundaries do not create T-joints.
        faces.extend([(rim[a],rim[b],wrap[b],wrap[a]),(wrap[a],wrap[b],back[b],back[a])])
        colors.extend([0,1 if -1.35<mid.x<.20 and mid.z>2.1 else 0])
    # Boundary can be re-ordered by triangulation; explicitly trace it for the
    # single planar inner face, instead of relying on CDT vertex ordering.
    adjacency={}
    for a,b in boundary:adjacency.setdefault(a,[]).append(b);adjacency.setdefault(b,[]).append(a)
    assert all(len(v)==2 for v in adjacency.values()),[(k,xy[k][:],v) for k,v in adjacency.items() if len(v)!=2]
    loop=[next(iter(adjacency))];prev=None;cur=loop[0]
    while True:
        nxt=next(v for v in adjacency[cur] if v!=prev)
        if nxt==loop[0]:break
        assert nxt not in loop,'Shoulder border contains multiple cycles'
        loop.append(nxt);prev,cur=cur,nxt
    faces.append(tuple(back[i] for i in loop));colors.append(0)
    o=mesh(name,verts,faces)
    for p,ci in zip(o.data.polygons,colors):p.material_index=ci;p.use_smooth=p.index<len(front)
    for e in o.data.edges:
        if tuple(sorted(e.vertices)) in boundary:e.use_edge_sharp=True
    o['surface_design']='Continuous convex shoulder; crown 0.52 author units; shared paint topology'
    o['crown_height_author_units']=.52
    bm=bmesh.new();bm.from_mesh(o.data)
    assert all(e.is_manifold for e in bm.edges),'Shoulder shell must be closed'
    bm.free()
    return o

def shoulder_surface_normals(o,scale,offset):
    # Analytic outward normals stay continuous through the constrained paint
    # triangles; skinny seam triangles must not make the broad armor lumpy.
    side=1 if o.name.endswith('_L') else -1
    normals=[tuple(n.vector) for n in o.data.corner_normals]
    for p in o.data.polygons:
        if not p.use_smooth:continue
        for li in p.loop_indices:
            point=o.data.vertices[o.data.loops[li].vertex_index].co/scale-offset
            dx,dz=point.x+.70,point.z-2.46
            along=.84*dx-.54*dz;across=.54*dx+.84*dz
            fx=-1.04*(along*.84/2.55**2+across*.54/1.50**2)
            fz=-1.04*(-along*.54/2.55**2+across*.84/1.50**2)
            normals[li]=tuple(Vector((-(fx+.14),side,-fz)).normalized())
...
def build_sweeper():
    ...
    def shoulders():
        start=len(PARTS)
        crowned_shoulder('Shoulder armor_L',shoulder)
        ...
        for v in PARTS[-1].data.vertices:
            v.co.y+=shoulder_crown(v.co.x,v.co.z)*max(0,min(1,(v.co.y-1.03)/.55))
        ...
        # Preserve the original forward lean, on top of the formed cross section.
    ...
    return dict(wheel_pivots_m=wheel_pivots,wheel_radius_m=.96,sockets_m={'Rig_GunPitch':list(pivot),'Muzzle_Gun':[3.925,0,2.96]},gun_pitch_limits_degrees=[-15,60],shoulder_surface='Mirrored convex formed shells; integral curved paint and seams',shoulder_crown_author_units=.52)

def build_war_machine_blockout():
...
def build_war_machine():
    ...
    from war_machine_reference import build
...
def main(unit):
    ...
    for o in PARTS:
        ...
        if o.name.startswith('Shoulder armor_'):shoulder_surface_normals(o,scale,offset)
