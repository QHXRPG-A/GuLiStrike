...
def main():
    out=Path(__file__).resolve().parents[2]/'Artifacts/Map3200/20260923'
    terrain=out/'terrain'
    records=[]
    for path in sorted(terrain.glob('weight-3200-*.png')):
        source=np.asarray(Image.open(path))
        persisted=np.asarray(Image.open(terrain/path.name.replace('weight-3200-','persisted-weight-')))
