...
def main():
    ...
    helpers={}
    exec((root/'Scripts/Map3200/capture_baseline.py').read_text(encoding='utf-8').rsplit('unreal.MCPythonHelper.submit_result',1)[0],helpers)
    value=helpers['value']
    ...
    assert definition.layout_version==4 and abs(definition.baked_object_scale-.2)<1e-5
    ...
    assert all(abs(v.x)<=135000 and abs(v.y)<=135000 for v in entity_positions)
    for marker in markers:
        r,c=marker['parameters']['BoardRow'],marker['parameters']['BoardColumn']
        x,y,_=marker['world_transform']['position']
        assert abs(x-(c-5)*30000)<1 and abs(y-(5-r)*30000)<1
    nav=[]
    for actor in actors:
        if actor.get_class().get_name() in ('NavMeshBoundsVolume','GuLiFlightNavigationVolume','RecastNavMesh'):
            ...
            if isinstance(actor,unreal.GuLiFlightNavigationVolume):
                assert abs(extent.x-212300)<1 and abs(extent.y-212300)<1
            ...
            if isinstance(actor,unreal.NavMeshBoundsVolume):
                assert abs(extent.x-135000)<1 and abs(extent.y-135000)<1
            nav.append(item)
    ...
    result={'success':True,'map':world.get_path_name(),'actors':len(actors),'landscape':value(info),
            ...
            'unique_outposts':81,'old_marker_and_region_ids_preserved':49,'new_outposts':32,
