...
if '-MechStyleReferenceWorker' not in unreal.SystemLibrary.get_command_line(): raise RuntimeError('Isolated worker required')
unreal.EditorPythonScripting.set_keep_python_script_alive(True)
unreal.EditorLoadingAndSavingUtils.new_blank_map(False)
OUT = Path('D:/UE5.7/test1/ArtSource/Mechs/StyleUnification_20260919/Source/Accessories')
    ...
    def mech_capture_tick(delta):
        try:
            ...
            if not st['configured']:
                ...
                floor.set_actor_location(unreal.Vector(origin.x,origin.y,origin.z-extent.z-8),False,False)
                cc.set_field_of_view(35. if view=='Hero' else 4.)
                pos=origin+unreal.Vector(*views[view])*span*(1. if view=='Hero' else 9.)
                ...
                cc.set_projection_mode(unreal.CameraProjectionMode.PERSPECTIVE)
