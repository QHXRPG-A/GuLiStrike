...
def main():
    ...
    identities=[]
    with unreal.ScopedEditorTransaction('Resize 81 stable outposts to the 1.8 km battlefield'):
        for entry in layout['markers']:
            actor=records[entry['marker_id'].replace('-','').lower()]
            row,col=entry['parameters']['BoardRow'],entry['parameters']['BoardColumn']
            key=f'Outpost_R{row}C{col}'
            assert entry['marker_key']==key
            actor.set_actor_location(ground((col-5)*20000,(5-row)*20000),False,False)
            identities.append({'marker_id':entry['marker_id'],'old_key':key,'new_key':key,
    ...
    for patch in json.loads((OUT/'density-patches-2300.json').read_text(encoding='utf-8')):
    ...
    for old in props:
        ...
        p=ground(centroid[0]*2/3+position[0]-centroid[0],centroid[1]*2/3+position[1]-centroid[1])
        p.z+=old['bounds']['extent'][2]-10
        actor.set_actor_location(p,False,False)
    assembly,factory=ground(0,72500),ground(0,87500)
    ...
    specs={
        'Note_0':(assembly+unreal.Vector(0,0,500),'2.3 km / 9×9 地图验收入口\n整张地形2.3×2.3 km，战场1.8×1.8 km；81据点，每格200m。红方R1C5，蓝方R9C5。\n正常指挥官玩法入口。双方各250个Mass、8矿车、8建造车；初始编队向战场内展开。\n先确认部队出现与框选就绪，再验证移动、S停止、B施工及运输。'),
        'Note_1':(factory+unreal.Vector(0,-1400,500),'采矿返厂\n工厂锚点(0,±87500 cm)，集合点(0,±72500 cm)。\n观察矿车采矿、返厂直接卸货、接续任务；采空矿簇应解除导航阻挡。'),
        'Note_2':(assembly+unreal.Vector(1800,0,500),'施工验收\n每队8建造车；B进入建造，在基地合法平地放置可负担建筑。\n每车自主接单：当前据点优先，同级最早工单优先；无本地单时接其他据点。\n途中不占位，到位抢位；发现占位冲突退单。外据点单途中可换为当前据点新单；已施工车暂停接单。\n验证四车协作、S停止、手动命令和完工通行。'),
        'Note_3':(ground(0,60000)+unreal.Vector(0,0,500),'据点推进与运输：R2C5\n红方从R1C5推进，沿用相邻占领、围合及运输规则。\n检查中央四向坡道、四角据点和易主后的运输通道。'),
    ...
    by_name['Actor_3'].set_actor_location(unreal.Vector(0,0,165000),False,False)
    for name in ('DirectionalLight_0','SkyLight_1','PostProcessVolume_2'):
        old=old_props[name]['transform']['location']
        by_name[name].set_actor_location(unreal.Vector(old[0]*23/32,old[1]*23/32,old[2]),False,False)
    ...
    result={'success':True,'preserved_markers':len(identities),'new_markers_pending_canonical_prepare':0,
