...
def main():
    ...
    target=json.loads((out/'terrain/layout-2300.json').read_text(encoding='utf-8'))
    ...
    assert abs(extent.x-160000)<1,'Terrain migration already applied; inspect before repeating.'
    for actor in actors:
        if isinstance(actor,unreal.NavMeshBoundsVolume):
            ...
            actor.set_actor_scale3d(unreal.Vector(scale.x*90000/extent.x,scale.y*90000/extent.y,scale.z*(high-low)/2/extent.z))
    ...
    land.set_actor_label('Landscape_2300')
    imported=unreal.LandscapeService.import_heightmap(land.get_name(),str(out/'terrain/height-2300.png'))
