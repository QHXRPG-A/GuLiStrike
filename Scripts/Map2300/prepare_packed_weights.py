...
def main():
    out=Path(__file__).resolve().parents[2]/'Artifacts/Map2300/20260923/terrain'
    target=json.loads((out/'layout-2300.json').read_text(encoding='utf-8'))
    weights=[np.asarray(Image.open(out/('weight-2300-'+layer['layer_name']+'.png')),dtype=np.uint8) for layer in target['source_layers']]
