...
class NetworkRun:
    def __init__(self, vehicle='ship'):
        ...
        self.out = Path(unreal.Paths.convert_relative_path_to_full(unreal.Paths.project_dir()))/'TestResults/CommanderTeleport'/((('dedicated' if self.mode==1 else 'listen')+'-'+vehicle)+'.json')
    ...
    def tick(self,delta):
        try:
            ...
            if self.stage==0 and elapsed>=2:
                ...
                self.report['checks']['server_collected']=state.phase==unreal.GuLiTeleportPhase.AWAITING_DESTINATION and state.participant_count>=(26 if self.vehicle=='ship' else 2)
                ...
                self.stage=2
            elif self.stage==2 and elapsed>=12:
                ...
                state=self.input.query_state()
                unreal.GuLiTeleportQALibrary.submit_intent(self.sender,unreal.GuLiTeleportCommand.DESTINATION,state.cast_id,self.source+(unreal.Vector(-16000,-8000,0) if self.vehicle=='ship' else unreal.Vector(8000,0,0)))
                self.stage=3
            elif self.stage==3 and elapsed>=13.3:
                ...
                self.stage=4
            elif self.stage==4 and elapsed>=16:
                ...
                self.report['checks']['displaced']=abs(self.ship.get_actor_location().x-self.source.x)>4000
