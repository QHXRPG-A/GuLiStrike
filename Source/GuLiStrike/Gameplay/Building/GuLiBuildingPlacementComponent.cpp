	...
	const bool bWorldInputBlockedByUI)
{
	...
	PreviewYawDegrees = FRotator::NormalizeAxis(YawDegrees);
	...
	FHitResult& OutHit) const
{
	...
	const FVector Start = DesiredLocation + FVector::UpVector
		* GuLiBuildingPlacement::ServerGroundTraceUpCentimeters;
	const FVector End = DesiredLocation - FVector::UpVector
...
bool UGuLiBuildingPlacementComponent::IsLegalGroundHit(const FHitResult& Hit) const
{
	...
	return Hit.bBlockingHit && HitComponent
		...
		&& Hit.GetActor() && !Hit.GetActor()->IsA<AGuLiPlacedBuilding>();
}
	...
	const FGuLiBuildingDefinition*& OutDefinition, AActor*& OutSupportingActor) const
{
	...
	const float YawDegrees = FRotator::NormalizeAxis(Request.GetYawDegrees());
	if (!IsPlacementAreaClear(*OutDefinition, GroundHit.ImpactPoint, YawDegrees, GroundHit.GetActor()))
	{
		return EGuLiBuildingPlacementRejectReason::Blocked;
	}
