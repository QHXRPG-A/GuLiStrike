...
#include "GuLiStrikeCharacter.generated.h"
...
protected:
	...
	int32 DashTrailAfterimageCount = 4;

	/** How long each trail afterimage remains visible. */
	UPROPERTY(EditAnywhere, Category="Dash|VFX", meta = (ClampMin = 0.01, ClampMax = 3.0, Units = "s"))
	float DashTrailAfterimageLifetime = 0.38f;

	/** Peak opacity applied to the afterimages nearest the blink destination. */
	...
	TSubclassOf<AGuLiStrikeAoEAttack> AoEAttackClass;
