...
uint8 Priority(const FValue& Value, const FValue* Previous)
{
	...
	if (Previous && ((Previous->State.ActiveOrderId && !Value.State.ActiveOrderId)
		...
		|| Previous->State.Team != Value.State.Team
	...
	if (Previous && Previous->Endpoint.IsSet() && !Value.Endpoint.IsSet()) return 1;
 if (Value.Endpoint.IsSet() && (!Previous || Previous->State.ActiveOrderId != Value.State.ActiveOrderId
  || !Previous->Endpoint.IsSet() || Previous->Endpoint->Revision != Value.Endpoint->Revision)) return 2;
 return 3;
}
...
bool FSender::Prepare(int32 MaxBytes, bool bUrgentOnly, double Now, FPrepared& Out)
{
	...
	for (int32 PriorityIndex=0; PriorityIndex<4; ++PriorityIndex)
	{
		...
		for (int32 I=Head; I<Q.Num() && Ids.Num()<MaxRecords; ++I)
		{
			...
			if (bUrgentOnly && PriorityIndex==3 && Now-P->FirstWaitingTime<1.0) break;
			Ids.Add(Q[I].Id);
		}
	}
	for (auto Id : Ids)
