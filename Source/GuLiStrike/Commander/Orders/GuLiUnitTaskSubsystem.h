...
public:
	...
	/** World-frame admission; only explicitly dirtied Mass StateTrees run at zero delta. */
...
private:
	...
	struct FAdmissionOwnerQueue { TArray<FGuLiTaskUnitId> Units; int32 Cursor=0; };
	TMap<TWeakObjectPtr<AGuLiBattlePlayerState>,FAdmissionOwnerQueue> AdmissionQueues;
	TArray<TWeakObjectPtr<AGuLiBattlePlayerState>> AdmissionOwners;
	uint32 AdmissionOwnerCursor=0;
	bool bPumpingAdmissions=false;
	TSet<FGuLiTaskUnitId> AdmissionQueued;
	TArray<FGuLiTaskUnitId> ReadyAdmissions;
	uint64 AdmissionFrame = MAX_uint64;
	int32 AdmissionRemaining = 0;
	uint64 ProgressFrame = MAX_uint64;
	void QueueMoveAdmission(FGuLiTaskUnitId Unit);
	...
	void StartMoveBatches(FGuLiNavigationWorkBudget& Budget);
