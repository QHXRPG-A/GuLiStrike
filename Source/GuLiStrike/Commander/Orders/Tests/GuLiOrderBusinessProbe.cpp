...
struct FRun
{
	...
	void WarpFactoryEntry()
	{
		const auto Entry = Factory->GetActorTransform().TransformPosition(Factory->GetDockRoute().Entry);
		Place(*Miner, Entry);
	}
	bool Tick(float)
	{
		...
		if (Stage == 1)
		{
			const auto* Def = World->GetSubsystem<UGuLiSpecialTaskCatalog>()->Find(2);
			FGuLiUnitTaskCommand Pick; Pick.Kind = EGuLiUnitTaskKind::Special; Pick.SpecialTaskId = 2;
			Check(TEXT("nearest_complete_ground_path_selected"), Def->Executor->BuildAutomatic(*World, StateOf(*Builder)->Context, Pick) && Pick.BuildingId == Life(*NearSite).GetState().InstanceId);
		...
		else if (Stage == 6)
		{
			...
			Check(TEXT("no_target_keeps_persistent_grant"), !StateOf(*Builder)->Grants.IsEmpty() && !StateOf(*Builder)->Grants[0].bConsumed);
			...
			Miner->ManeuverSpeed = 3000; Miner->DockingSeconds = .2f;
			Send(*Miner, EGuLiTaskDisposition::Replace, EGuLiUnitTaskKind::ReturnToFactory);
			Next(7);
		}
		else if (Stage == 7)
		{
			if (Miner->GetTaskState() == EGuLiMiningTaskState::ReturningToFactory) WarpFactoryEntry();
			if (!Miner->IsFactoryManeuverActive()) return true;
			Check(TEXT("return_factory_real_maneuver_started"), true);
			Send(*Miner, EGuLiTaskDisposition::Stop);
			Check(TEXT("factory_stop_waits_for_safe_exit"), StateOf(*Miner)->bStopped && StateOf(*Miner)->bCancelPending && Miner->IsFactoryManeuverActive());
		...
		{
			if (Miner->IsFactoryManeuverActive() || StateOf(*Miner)->bCancelPending) return true;
			Check(TEXT("factory_exited_before_latest_stop_took_effect"), StateOf(*Miner)->bStopped && !StateOf(*Miner)->Active.IsSet() && Miner->GetCargoTotal() == 0);
		...
		else if (Stage == 11)
		{
			...
			Check(TEXT("mining_full_round_yields_after_unload_and_exit"), Miner->GetCargoTotal() == 0 && !Miner->IsFactoryManeuverActive());
			Send(*Miner, EGuLiTaskDisposition::Stop);
		...
		else if (Stage == 15)
		{
			...
			Check(TEXT("new_soldier_gets_own_initial_grant"), A && A->Active.IsSet() && !A->Grants[0].bConsumed);
			...
			Check(TEXT("advance_append_does_not_consume_until_stage_boundary"), !A->Grants[0].bConsumed && A->Active->bYieldRequested);
		...
		else if (Stage == 16)
		{
			...
			Check(TEXT("advance_boundary_consumes_only_handed_over_member"), A->Grants[0].bConsumed && !B->Grants[0].bConsumed);
			Tasks().RegisterSoldiers(EGuLiTeam::Red, NewSoldiers); Tasks().NotifyAdvanceStage(NewSoldiers[0]);
			Check(TEXT("stale_stage_callback_cannot_restore_initial_task"), A->Grants[0].bConsumed);
			...
			Check(TEXT("mixed_selection_accepts_only_capable_unit"), Accepted == 1 && Rejected == 2 && StateOf(*Builder)->bStopped && !B->Grants[0].bConsumed
		...
		{
			if (Miner->IsFactoryManeuverActive() || StateOf(*Miner)->Error.IsEmpty()) return true;
			Check(TEXT("factory_ownership_loss_preserves_cargo_and_reports_failure"), Miner->GetCargoTotal() == 2);
