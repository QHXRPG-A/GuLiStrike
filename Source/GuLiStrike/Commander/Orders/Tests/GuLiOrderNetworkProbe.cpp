		...
		if (!Client)
		{
			...
			if (SawTwo && Driver && Driver->ClientConnections.IsEmpty())
			{
				...
				for (const auto& Soldier:Soldiers) if (Soldier.IsAlive())
				{
					...
					const bool Used=Before->Grants.ContainsByPredicate([](const auto& G){ return G.Lifetime==EGuLiTaskLifetime::InitialOnce && G.bConsumed; });
					...
					Stable &= After && After->bStopped==WasStopped && Used==After->Grants.ContainsByPredicate([](const auto& G){ return G.Lifetime==EGuLiTaskLifetime::InitialOnce && G.bConsumed; });
				}
