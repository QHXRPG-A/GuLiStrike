...
void UGuLiCommanderHUDWidget::RefreshConsoleContext()
{
	...
	const auto* Tasks=GetWorld()->GetSubsystem<UGuLiSpecialTaskCatalog>();
	...
	{
		Miners+=Tasks && Tasks->Find(FGameplayTag::RequestGameplayTag(TEXT("Task.Special.Mining")),Row.Type)?1:0;
		Builders+=Tasks && Tasks->Find(FGameplayTag::RequestGameplayTag(TEXT("Task.Special.Construction")),Row.Type)?1:0;
