...
def finish_factory(asset):
    if unreal.EditorAssetLibrary.get_metadata_tag(asset,'GuLi.RPF.InterfaceRevision')=='2':return asset
    ...
    report=compile_bp(BP)
    unreal.EditorAssetLibrary.set_metadata_tag(asset,'GuLi.RPF.InterfaceRevision','2');save(asset)
    (OUT/'ue_blueprint_report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
    return asset
...
def main():
    ...
    parser.add_argument('--phase',choices=['assets','map'],default='assets')
