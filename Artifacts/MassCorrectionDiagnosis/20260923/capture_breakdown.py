...
assert not (_nb_root / 'breakdown-start.json').exists(), 'Do not overwrite evidence'
...
if _nb_previous_sample is not None:
    (_nb_root / 'breakdown-preexisting-pie-sample.json').write_bytes(_nb_previous_sample)
...
_nb_sample('breakdown-counters-start.json')
...
unreal.log('GULI_NETWORK_BREAKDOWN_BEGIN frame={}'.format(_nb_meta['start_frame']))
_nb_command('Log LogNet VeryVerbose')
_nb_command('getall IpConnection CurrentNetSpeed')
_nb_command('netprofile AUTOSTOP TIME=32')
_nb_command('CsvProfile STARTFILE=../../../Artifacts/MassCorrectionDiagnosis/20260923/breakdown-engine.csv')
...
def _nb_finish(error=None):
    ...
    _nb_sample('breakdown-counters-end.json')
    if _nb_previous_sample is not None:
    ...
    (_nb_root / 'breakdown-capture.json').write_text(json.dumps({'meta': _nb_meta, 'frames': _nb_frames}, indent=2), encoding='utf-8')
    unreal.log('GULI_NETWORK_BREAKDOWN_END frame={}'.format(_nb_meta['end_frame']))
...
def _nb_tick(delta):
    ...
    try:
        ...
        if marker != _nb_last_marker:
            ...
            unreal.log('GULI_NETWORK_BREAKDOWN_TICK frame={} elapsed={:.6f}'.format(frame, elapsed))
        if elapsed >= 30:
            _nb_finish()
...
(_nb_root / 'breakdown-start.json').write_text(json.dumps(_nb_meta, indent=2), encoding='utf-8')
