...
capture = json.loads((OUT / 'loaded-capture.json').read_text())
...
log = (OUT/'loaded-network.log').read_text()
...
counter_start=json.loads((OUT/'loaded-counters-start.json').read_text())
counter_end=json.loads((OUT/'loaded-counters-end.json').read_text())
...
report['notes']=[
    ...
    'Profile is process-wide. The server-port destination combines both client uplinks; the other two destinations are separate server downlinks.',
    ...
    'Last partial frame excluded. Frame-index mapping checked against begin/end GFrameCounter.',
    ...
    'User restarted PIE before this capture. Native read-only census inside the capture at 14:54:57: alive=600 active=600.',
...
report['sha256']={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (
    OUT/'loaded-network.nprof',OUT/'loaded-network.log',OUT/'loaded-engine.csv')}
(OUT/'loaded-analysis.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
columns=['frame','connection','engine_seconds']+sorted(set().union(*(r.keys() for r in rows))-{'frame','connection','engine_seconds'})
with (OUT/'loaded-frames.csv').open('w',encoding='utf-8',newline='') as f:
