...
root = Path('D:/UE5.7/test1/Artifacts/MassCorrectionDiagnosis/20260923')
outputs = Path('D:/UE5.7/test1/outputs/mass-correction-diagnosis-20260923')
outputs.mkdir(parents=True, exist_ok=True)
result = {'pid': os.getpid(), 'wall_time': time.time(), 'worlds': []}
unreal.log('GULI_CONTINUOUS_CORRECTION_DIAG_CONTEXT_BEGIN')
for index in range(3):
    ...
    if index == 0:
        unreal.SystemLibrary.execute_console_command(world, 'gs.GM.Commander.Nav.Stats')
        ...
        snapshot = outputs / ('client-{}.json'.format(index))
        ...
        row['presentation'] = {name: presentation.get_editor_property(name) for name in (
            ...
            'maximum_extrapolation_seconds', 'maximum_correction_speed_multiplier')}
...
unreal.log('GULI_CONTINUOUS_CORRECTION_DIAG_CONTEXT_END')
result['cvars'] = {name: unreal.SystemLibrary.get_console_variable_float_value(name) for name in (
    't.MaxFPS', 'r.VSync', 't.IdleWhenNotForeground', 'r.ScreenPercentage')}
