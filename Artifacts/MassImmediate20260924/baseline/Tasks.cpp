	...
	const FGuLiUnitTaskCommand& Command, FString& Message, int32& Accepted, int32& Rejected, TSet<FGuLiTaskUnitId>* AcceptedUnits)
{
	...
	for (auto Unit : Members(Selection))
	{
		...
		if (!AdmitCommand(*State, Command, Error)) { ++Rejected; Message = Error; continue; }
...
{
	if (AdmissionFrame == GFrameCounter) { FGuLiNavigationWorkBudget::FScope Scope(Budget); StartMoveBatches(Budget); return; }
	AdmissionFrame = GFrameCounter;
	...
	const int32 Count=FMath::Min(25,AdmissionQueued.Num());
...
void UGuLiUnitTaskSubsystem::StartMoveBatches(FGuLiNavigationWorkBudget& Budget)
{
	...
	while (!ReadyAdmissions.IsEmpty() && Budget.CanWork())
	{
		...
		FGuLiCommandAck Ack;
		if (!Authority->BeginMovePlanning(*Batch.Owner,Request,Frozen,Ack))
		{
