...
namespace GuLiResourceAuthoring
{
	...
	constexpr float OutpostExclusionRadius = 10000.0f;
	...
	bool IsExplicitlyExcluded(const FVector2D Location)
	{
			...
			&& FMath::Abs(Location.Y) <= GULI_RESOURCE_PLAYABLE_HALF_EXTENT_CM)
		{
			...
			if (FMath::Abs(Location.X - RoadX) < BoardRoadHalfWidth
				|| FMath::Abs(Location.Y - RoadY) < BoardRoadHalfWidth) return true;
		}
		...
		FString& OutError)
	{
		...
		for (int32 TerritoryIndex = 0; TerritoryIndex < GULI_RESOURCE_TERRITORY_COUNT; ++TerritoryIndex)
		{
			...
			// red pairs even though a valid 10-cluster packing exists on the 25 m grid.
			...
			TFunction<bool(int32, int32)> Choose = [&](int32 Depth, int32 First) -> bool
			{
				...
				for (int32 Index = First; Index < Candidates[TypeIndex].Num(); ++Index)
				{
