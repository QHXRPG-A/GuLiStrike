...
static bool ConstructionBuildAutomatic(UWorld& World, const FGuLiTaskUnitContext& Unit, FGuLiUnitTaskCommand& Command)
{
    ...
    for (int32 Pass=0; Pass<2; ++Pass)
    {
        ...
        bool bHaveAvailable=false;
        for (const auto* Building : Buildings)
        {
            if ((Building->GetState().TerritoryIndex==Current)!=(Pass==0)) continue;
            FVector Position; float Length;
            if (!Work->PrepareBuilding(*Building,Position,Length) || Work->AreAllPositionsRejected(*Building)) continue;
            const bool Available=Building->GetConstructionAvailability()==EGuLiWorkPositionAvailability::Available;
            if ((Available && !bHaveAvailable) || (Available==bHaveAvailable && Length<Best))
            { bHaveAvailable=Available; Best=Length; Command.BuildingId=Building->GetState().InstanceId; Command.Target=Position; }
        }
        if (Command.BuildingId) break;
    }
