...
bool UGuLiResourceMapDefinition::ValidateDefinition(FString& OutError) const
{
		...
		|| !PlayableMaximum.Equals(FVector2D(GULI_RESOURCE_PLAYABLE_HALF_EXTENT_CM), 0.1f))
	{
		return FailResourceDefinition(OutError, FString::Printf(
			TEXT("Playable bounds must be the canonical %.0f x %.0f cm square."),
			2.0f * GULI_RESOURCE_PLAYABLE_HALF_EXTENT_CM, 2.0f * GULI_RESOURCE_PLAYABLE_HALF_EXTENT_CM));
	}
	...
	for (int32 Index = 0; Index < Territories.Num(); ++Index)
	{
		...
		const EGuLiTeam ExpectedOwner = GuLiResources::GetInitialTerritoryOwner(Territory.BoardRow, Territory.BoardColumn);
	...
	for (int32 ClusterIndex = 0; ClusterIndex < Clusters.Num(); ++ClusterIndex)
	{
		...
		const float LegalHalfExtent = GULI_RESOURCE_TERRITORY_HALF_EXTENT_CM - GULI_RESOURCE_TERRITORY_BOUNDARY_MARGIN_CM;
		if (FMath::Abs(Local.X) > LegalHalfExtent || FMath::Abs(Local.Y) > LegalHalfExtent)
		{
			return FailResourceDefinition(OutError, FString::Printf(TEXT("Cluster violates the %.1f m Territory boundary margin."),
				GULI_RESOURCE_TERRITORY_BOUNDARY_MARGIN_CM / 100.0f));
		}
...
{
	OutError.Reset();
	if (MiningVehicleUnitTypeId <= 0 || MiningVehicleUnitTypeId > MAX_uint16
		|| InitialMiningVehiclesPerTeam < 0 || InitialConstructionVehiclesPerTeam < 0
