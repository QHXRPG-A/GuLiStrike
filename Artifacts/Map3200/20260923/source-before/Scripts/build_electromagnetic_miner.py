...
def build_preserved_vehicle():
    ...
    vars=[('VehicleBodyLengthCm','float','591.6596'),('MiningRangeBodyLengths','float','0.0'),
          ('MiningRangeCm','float','0.0'),('MiningActive','bool','False'),
    ...
    assert unreal.NiagaraService.save_system(FX)
    record('asset-paths.json',dict(vehicle=BP,collector=mesh,beam=FX,source=SOURCE,range_cm=None,beam_endpoint='MiningTargetWorld'))
...
def build_graphs():
    ...
    for side in ['L','R']:
        ...
        g.edge('beam','MiningLaser_'+side,'endpoint','self');g.edge('target','ReturnValue','endpoint','InValue');g.val('endpoint','InVariableName','User.Beam End')
        ...
        S.add_comment_around_nodes(BP,g.name,'Aim collector at target. User.Beam End follows the complete world-space target with no distance clamp.',list(g.nodes.values()))
    ...
    g.branch('branch');g.edge('allowed','ReturnValue','branch','Condition');g.chain('entry','branch')
    ...
    assert S.modify_variable(BP,'MiningRangeBodyLengths',new_category='Mining|Legacy',new_tooltip='Unused legacy limit. Beam length now follows the mining target.')
