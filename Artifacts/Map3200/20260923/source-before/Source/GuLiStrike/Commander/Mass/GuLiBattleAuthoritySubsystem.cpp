...
bool UGuLiBattleAuthoritySubsystem::TrySpawnAuthorityPopulation()
{
	...
	ResourceAdapter->GetInitialArmySpawnAnchor(EGuLiTeam::Red, RedAssembly);
	...
	for (int32 TeamIndex = 0; Deployments.IsEmpty() && TeamIndex < TeamCount && bSpawnValidationSucceeded; ++TeamIndex)
	{
		...
		const FVector TeamCenter = Team == EGuLiTeam::Red ? RedAssembly : BlueAssembly;
		const FVector OpposingCenter = Team == EGuLiTeam::Red ? BlueAssembly : RedAssembly;
