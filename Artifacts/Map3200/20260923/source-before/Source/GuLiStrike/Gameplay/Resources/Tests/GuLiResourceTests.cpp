...
namespace GuLiResourceTests
{
	...
	TArray<EGuLiTeam> MakeInitialOwners()
	{
		...
		Owners[GuLiResources::ToTerritoryIndex(1, 4)] = EGuLiTeam::Red;
		Owners[GuLiResources::ToTerritoryIndex(7, 4)] = EGuLiTeam::Blue;
		return Owners;
	}
}
...
bool FGuLiResourceBoardContractTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("R1C4 is the north home center"),
		GuLiResources::GetTerritoryCenter(1, 4), FVector(0.0, 120000.0, 0.0));
	TestEqual(TEXT("R7C4 is the south home center"),
		GuLiResources::GetTerritoryCenter(7, 4), FVector(0.0, -120000.0, 0.0));
	return true;
}
...
bool FGuLiResourceFastArrayStateTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Only north home begins red"),
		State->GetTerritoryOwner(GuLiResources::ToTerritoryIndex(1, 4)), EGuLiTeam::Red);
	TestEqual(TEXT("Only south home begins blue"),
		State->GetTerritoryOwner(GuLiResources::ToTerritoryIndex(7, 4)), EGuLiTeam::Blue);
