...
{
    RemainingMoveQueries = 4; // Same per-world-frame path query budget as before.
    auto& Resources = *GetWorld()->GetSubsystem<UGuLiResourceWorldSubsystem>();
...
void UGuLiArmyAdvanceSubsystem::MoveToBehaviorTarget(FGuLiSoldierId Soldier)
{
    ...
    const bool bAccepted = Authority.IssueAttackMove(Group->Team, Group->Soldiers, Group->Approach);
    Group->Phase = EGuLiCommanderWorkPhase::AdvanceMoving;
    Group->Result = bAccepted ? EGuLiCommanderWorkResult::Running : EGuLiCommanderWorkResult::Failed;
