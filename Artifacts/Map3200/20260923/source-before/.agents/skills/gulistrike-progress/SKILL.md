...
从项目外部复制、导入或迁移资源时，增量归档必须包含一节“资源清单”。清单按**最终项目内文件夹**列出来源路径、目标路径、该文件夹的大致内容与用途，以及依赖、示例地图、外部 Actor/Object 支持目录、未迁入内容或验证边界；以能定位和复用资源为准，不逐个枚举 `.uasset`。目录重组或重命名后以最终路径为主，并保留原包名称或来源映射。

清单信息优先来自压缩包目录、文件路径和 Unreal Asset Registry/编辑器查询；遵守项目二进制资产禁读规则，不直接读取 `.uasset`、`.umap`、`.uexp`、`.ubulk` 或 `.pak` 内容。创建此类归档时使用 [`references/templates.md`](references/templates.md) 的条件模板，并按 [`references/archive-policy.md`](references/archive-policy.md) 记录可复现路径与验证边界。
