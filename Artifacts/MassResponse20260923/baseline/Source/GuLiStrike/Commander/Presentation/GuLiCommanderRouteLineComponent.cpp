...
struct FLineChunkUpdate { int32 Index=0; TArray<FVector3f> Positions; };
}
...
public:
		...
		: FPrimitiveSceneProxy(Component)
	{
		...
		for (int32 I=0; I<Component->Chunks.Num(); ++I)
		{
			...
			{ U.Positions.Add(FVector3f(Component->GetComponentTransform().InverseTransformPosition(Line.Start))); U.Positions.Add(FVector3f(Component->GetComponentTransform().InverseTransformPosition(Line.End))); }
		}
...
void UGuLiCommanderRouteLineComponent::SendRenderDynamicData_Concurrent()
{
	...
	for (int32 I : DirtyChunks)
	{
		...
		{ U.Positions.Add(FVector3f(GetComponentTransform().InverseTransformPosition(L.Start))); U.Positions.Add(FVector3f(GetComponentTransform().InverseTransformPosition(L.End))); }
	}
...
void UGuLiCommanderRouteLineComponent::TickComponent(float Dt,ELevelTick Tick,FActorComponentTickFunction* Function)
{
		...
		&& FPlatformTime::Seconds()-Start<FMath::Max(.01f,CVarRouteLineBudgetMs.GetValueOnGameThread())*.001)
	{
		...
		if (!L.bVisible || L.Start!=A || L.End!=B) { L.Start=A; L.End=B; L.Order=E.ActiveOrderId; L.bVisible=true; DirtyChunks.Add(Slot/LinesPerChunk); }
	}
