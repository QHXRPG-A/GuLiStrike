...
bool UGuLiCommanderResourceAdapter::GetInitialArmySpawnAnchor(
	const EGuLiTeam Team, FVector& OutAnchor, float* OutInwardInset) const
{
	const UGuLiResourceWorldSubsystem& ResourceWorld = Resources();
	const UGuLiResourceMapDefinition* Definition = ResourceWorld.GetMapDefinition();
	if (!ResourceWorld.IsResourceWorldActive() || !Definition
		|| (Team != EGuLiTeam::Red && Team != EGuLiTeam::Blue)) return false;
	OutAnchor = Team == EGuLiTeam::Red
		? Definition->SpawnAnchors.RedAssembly : Definition->SpawnAnchors.BlueAssembly;
	if (OutInwardInset) *OutInwardInset = GULI_RESOURCE_INITIAL_ARMY_INSET_CM;
	return true;
}
	...
	const FGuLiMiningCommand& Command) const
{
	...
	for (const FGuLiControllableActorId Id : SelectedIds)
	{
		...
		{
			FGuLiUnitTaskCommand Task; Task.CommandId = Command.RequestId; Task.SelectionRevision = Command.SelectionRevision;
			Task.Target = Command.Target;
			auto* Tasks = GetWorld()->GetSubsystem<UGuLiUnitTaskSubsystem>();
			if (Command.Type == EGuLiMiningOrderType::Cancel)
			{ Task.Disposition = EGuLiTaskDisposition::Stop; bAcceptedAny |= Tasks->SubmitActorCommand(*Builder, Task); continue; }
			...
			if (Site) { Task.Kind = EGuLiUnitTaskKind::Special; Task.SpecialTaskId = 2; Task.BuildingId = Site->GetState().InstanceId; }
			bAcceptedAny |= Tasks->SubmitActorCommand(*Builder, Task);
		}
	}
	return bAcceptedAny;
}
