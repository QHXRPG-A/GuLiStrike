...
namespace GuLiCommanderDebugCommands
{
	...
	void LogSoldierNavigation(const TArray<FString>& Args, UWorld* World)
	{
		...
		UE_LOG(
			LogGuLiStrike,
			Display,
			TEXT("Commander nav soldier=%u team=%u state=%s moving=%d order=%u completed=%u failed_order=%u failure=%s failure_time=%.3fs location=%s last_nav=%s nav_ref=%llu nav_ref_valid=%d nav_generation=%u final=%s distance=%.1fcm waypoint=%s path_index=%d no_progress=%.3fs surface_failures=%d/%d personal_queries=%d combat_target=%u cooldown=%.3fs combat_stop=%s."),
			...
			*Navigation.LastValidNavLocation.ToCompactString(),
			Navigation.NavigationNodeRef,
			Navigation.bNavigationNodeRefValid ? 1 : 0,
	...
	void LogNavigationStats(const TArray<FString>& Args, UWorld* World)
	{
		...
		UE_LOG(
			LogGuLiStrike,
			Display,
			TEXT("Commander nav stats alive=%d active=%d idle=%d arrived=%d centerline=%d personal=%d blocked=%d surface=%llu failed=%llu movement_updates=%llu forced_updates=%llu avoidance_refreshes=%llu avoidance_candidates=%llu avoidance_overlaps=%llu avoidance_max_bucket=%d predictive_solves=%llu predictive_forced=%llu predictive_candidates=%llu predictive_colliders=%llu predictive_max_bucket=%d paths=%llu personal_paths=%llu pending_plans=%d candidate_projections=%llu planning_paths=%llu partial_moves=%llu plan_failures={%s} plan_ms=%.3f max_plan_ms=%.3f slowest=%u no_progress=%.3fs."),
			Stats.Alive,
			Stats.Active,
			Stats.Idle,
			Stats.Arrived,
			...
			Stats.MovementUpdateCalls,
			Stats.ForcedMovementUpdateCalls,
	...
	void LogLastMovePlanning(const TArray<FString>& Args, UWorld* World)
	{
		...
		UE_LOG(
			LogGuLiStrike,
			Display,
			TEXT("Commander nav last_move command=%u batch=%u target=%s theoretical=%d projected=%d legal=%d desired_legal=%d prefix=%d->%d expansions=%d full_pool=%s planning_frames=%d candidate_queries=%d path_queries=%d route_splits=%d reservation_conflicts=%d accepted=%d failed=%d max_radius=%.1fcm plan_ms=%.3f failed_ids={%s} failures={%s}."),
			Debug.ClientCommandId,
			Debug.BatchOrderId,
			*Debug.RequestedTarget.ToCompactString(),
			...
			Debug.bEscalatedToFullCandidatePool ? TEXT("true") : TEXT("false"),
