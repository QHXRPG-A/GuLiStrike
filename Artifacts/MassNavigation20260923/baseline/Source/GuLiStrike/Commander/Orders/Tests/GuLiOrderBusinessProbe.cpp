...
struct FRun
{
	...
	TWeakObjectPtr<AGuLiPlacedBuilding> OldSite, NewSite, UnreachableSite, EnemySite, EqualSite;
	...
	AGuLiPlacedBuilding* Site(FVector At, EGuLiTeam Team = EGuLiTeam::Red)
	{
		...
		Actor->InitializeFromDefinition(Def.DefinitionId, Team, FGuid::NewGuid(), Resources().FindTerritoryIndex(At), EGuLiBuildingOrigin::Manual, false);
	...
	bool Tick(float)
	{
		...
		if (Stage == 0)
		{
			...
			Origin = Ground(GuLiResources::GetTerritoryCenter(2, 5) + FVector(-6000, 0, 0), *Builder); Place(*Builder, Origin);
			...
			OldSite = Site(Ground(Origin + FVector(8000,0,0), *Builder));
			EqualSite = Site(Life(*OldSite).GetGroundLocation());
			NewSite = Site(Ground(Origin + FVector(4000,0,0), *Builder));
		...
		if (Stage == 1)
		{
			...
			Check(TEXT("oldest_available_site_selected_before_nearer_new_site"), GuLiCommanderAbilities::BuildAutomatic(EGuLiCommanderBehavior::Construction, *World, StateOf(*Builder)->Context, Pick) && Pick.BuildingId == Life(*OldSite).GetState().InstanceId);
			FVector EqualPosition, NearPosition; float EqualLength, NearLength;
			Check(TEXT("creation_order_does_not_reserve_a_position"), Work(*Builder).PrepareBuilding(Life(*OldSite), NearPosition, NearLength)
				&& Work(*Builder).PrepareBuilding(Life(*EqualSite), EqualPosition, EqualLength) && Life(*OldSite).GetReservedConstructionSlots() == 0
			...
			if (!Work(*Builder).PrepareBuilding(Life(*OldSite), Position, Length)) { Finish(TEXT("fixture site unreachable")); return false; }
		...
		{
			if (Work(*Builder).GetTarget() != &Life(*OldSite) || Work(*Helper).GetTarget() != &Life(*OldSite)) return true;
			Check(TEXT("multiple_builders_share_site"), Life(*OldSite).GetState().WorkDone > 0);
			SavedWork = Life(*OldSite).GetState().WorkDone;
		...
		else if (Stage == 3)
		{
			...
			Check(TEXT("other_builder_continues_and_progress_retained"), Life(*OldSite).GetState().WorkDone >= SavedWork && Work(*Helper).GetTarget());
			Send(*Helper, EGuLiTaskDisposition::Stop);
			Send(*Builder, EGuLiTaskDisposition::Replace, EGuLiUnitTaskKind::Move, Builder->GetActorLocation()); Next(4, 2);
		}
		else if (Stage == 4)
		{
			if (Work(*Builder).GetTarget() != &Life(*OldSite)) return true;
			...
			Check(TEXT("construction_append_keeps_current_work"), Work(*Builder).GetTarget() == &Life(*OldSite) && StateOf(*Builder)->Active->bYieldRequested);
			Life(*OldSite).AddConstructionWork(Life(*OldSite).GetDefinition().ConstructionWork);
			Next(5, 2);
		}
		else if (Stage == 5)
		{
			Check(TEXT("completed_building_yields_to_manual_queue"), StateOf(*Builder)->Queue.IsEmpty() && (!StateOf(*Builder)->Active.IsSet() || StateOf(*Builder)->Active->Command.BuildingId != Life(*OldSite).GetState().InstanceId));
			Check(TEXT("new_site_command_accepted"), Send(*Builder, EGuLiTaskDisposition::Replace, EGuLiUnitTaskKind::Special, NewSite->GetActorLocation(), Life(*NewSite).GetState().InstanceId));
			Send(*Builder, EGuLiTaskDisposition::Append, EGuLiUnitTaskKind::Move, Builder->GetActorLocation());
			NewSite->Destroy(); Next(6, 2);
		}
		else if (Stage == 6)
		{
		...
		else if (Stage == 17)
		{
			...
			NewSite = Site(Ground(Builder->GetActorLocation()+FVector(4000,0,0), *Builder));
			Next(18, 2);
		}
		else if (Stage == 18)
		{
			if (!Send(*Builder, EGuLiTaskDisposition::Replace, EGuLiUnitTaskKind::Special, NewSite->GetActorLocation(), Life(*NewSite).GetState().InstanceId)) return true;
			Next(19);
		}
		else if (Stage == 19)
		{
			if (Work(*Builder).GetTarget() != &Life(*NewSite)) return true;
			NewSite->SetBuildingTeamAuthority(EGuLiTeam::Blue); Next(20);
		}
		else if (Stage == 20)
		{
