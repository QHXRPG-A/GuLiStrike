...
void UGuLiBattleAuthoritySubsystem::Tick(const float DeltaTime)
{
	...
	FGuLiNavigationWorkBudget::FAllowance Shares[4] = {
		{.0012,40,2}, {.0003,8,1}, {.0003,8,1}, {.0002,8,0}};
	auto RunCategory = [&](int32 Category, FGuLiNavigationWorkBudget::FAllowance& Allowance)
	{
		...
		else if (Category == 1) TickMovePlanning(Budget.Projections,Budget.Paths,false);
		else if (Category == 2) TickNavigationRepairs(Budget.Projections,Budget.Paths);
		else TickSteeringValidation();
	};
	// Reservations first, then borrow unused counters. Background first-in-round rotates each frame.
	for (int32 Round=0; Round<8 && Budget.CanWork(); ++Round)
	{
		RunCategory(0,Shares[0]);
		for (int32 Offset=0; Offset<3; ++Offset)
		{ const int32 Category=1+(GFrameCounter+Offset)%3; RunCategory(Category,Shares[Category]); }
	}
	for (int32 Category=0; Category<4; ++Category) AuthorityState->CategoryUsage[Category]=Shares[Category];
	for (int32 Round=0; Round<8 && Budget.CanWork() && (Budget.Projections>0 || Budget.Paths>0); ++Round)
		for (int32 Offset=0; Offset<4 && Budget.CanWork(); ++Offset)
		{
			const int32 Category=(GFrameCounter+Offset)%4;
			FGuLiNavigationWorkBudget::FAllowance Borrowed{Budget.LimitSeconds-Budget.Elapsed(),Budget.Projections,Budget.Paths};
			RunCategory(Category,Borrowed);
			auto& Used=AuthorityState->CategoryUsage[Category];
			Used.UsedSeconds+=Borrowed.UsedSeconds; Used.UsedProjections+=Borrowed.UsedProjections; Used.UsedPaths+=Borrowed.UsedPaths;
		}
	TickLocalFlowFields();
...
void UGuLiBattleAuthoritySubsystem::PrepareNextMoveBatch(const uint64 PlanId)
{
	...
	Job.NavigationGeneration = 0;
	Job.DependencyCursor = 0; Job.bCommitReady=false; Job.bExpandingFallback=false;
...
void UGuLiBattleAuthoritySubsystem::TickMovePlanning(
	...
	int32& RemainingPathBudget, const bool bManualFirst)
{
	...
	const int32 Category = bManualFirst ? 0 : 1;
	bool bWorked = false;
	const bool bOtherOwnerWaiting = bManualFirst && AuthorityState->MovePlanningJobs.ContainsByPredicate([&](const auto& J)
	{ return J && !J->bAutomatic && !J->SharedBatchOrderId && J->Stage!=EMovePlanningStage::Completed
		&& J->PlayerState != AuthorityState->LastManualOwner; });

	for (int32 Visit = 0, Count = AuthorityState->MovePlanningJobs.Num(); Visit < Count; ++Visit)
	{
		const auto& JobPointer = AuthorityState->MovePlanningJobs[(AuthorityState->PlanningCursor[Category] + Visit) % AuthorityState->MovePlanningJobs.Num()];
		...
		if (bWorked) break;
		if (Job.SourceCommandId)
			if (const auto* Group=AuthorityState->CommandGroups.Find({FObjectKey(Job.PlayerState.Get()),Job.SourceCommandId}))
				Job.SharedBatchOrderId=Group->Batch;
		if (bManualFirst != (!Job.bAutomatic && !Job.SharedBatchOrderId)) continue;
		if (bOtherOwnerWaiting && Job.PlayerState==AuthorityState->LastManualOwner) continue;
		...
		if (!AuthorityState->PlanningBudget.CanWork()) break;
		if (Job.bCommitReady && Job.NavigationGeneration==AuthorityState->NavigationGeneration) continue;
		bWorked=true;
		AuthorityState->PlanningCursor[Category]=(AuthorityState->PlanningCursor[Category]+Visit+1)%AuthorityState->MovePlanningJobs.Num();
		if (bManualFirst) AuthorityState->LastManualOwner=Job.PlayerState;
		Job.LastScheduledAt=FPlatformTime::Seconds();
		if (!Job.bFirstWorkRecorded)
		{
			Job.bFirstWorkRecorded=true;
			auto Trace=GuLiMoveLatency::Context(Job.PlayerState.Get(),Job.SourceCommandId); Trace.Epoch=Job.AuthorityEpoch; Trace.Execution=Job.Request.ClientCommandId; Trace.Plan=Job.Handle.Value;
			GuLiMoveLatency::Record(TEXT("first-work"),Trace,Job.FullSelection.Cohorts.Num()*25);
		}
		if (!Job.bCandidatesGenerated)
		{
			FMoveStageScope Profile(Job.StageProfile[0],AuthorityState->PlanningBudget);
		...
		if (Job.NavigationData.Get()!=NavigationData)
		{
			Job.ProjectionCache.Reset(); Job.InvalidProjectionCache.Reset();
			for (auto& Pair : Job.ProjectedNavByCandidateIndex) Pair.Value.NodeRef=0;
			Job.NavigationData=NavigationData;
		}
		if (!Job.PreparedFormations.IsEmpty())
		{
			FMoveStageScope Profile(Job.StageProfile[3],AuthorityState->PlanningBudget);
			if (!ValidatePreparedMove(Job,*NavigationData,AuthorityState->NavigationGeneration,AuthorityState->PlanningBudget,
				*World->GetSubsystem<UGuLiDynamicObstacleRegistrySubsystem>()->GetSnapshot(),AuthorityState->Soldiers,AuthorityState->SoldierIndexById)) { Job.bCommitReady=false; continue; }
			if (Job.Stage==EMovePlanningStage::ReadyToCommit)
			{
				Job.bCommitReady=!Job.PreparedFormations.IsEmpty() || Job.RouteTasks.IsEmpty();
				if (!Job.bCommitReady) Job.Stage=EMovePlanningStage::Route;
			}
		}

		if (Job.Stage == EMovePlanningStage::ValidateStarts)
		{
			FMoveStageScope Profile(Job.StageProfile[2],AuthorityState->PlanningBudget);
			...
			if (Job.NextStartValidationIndex >= Job.Members.Num())
			{
				...
				{
					Job.Stage = EMovePlanningStage::ReadyToCommit; Job.bCommitReady=true; Job.NavigationGeneration=AuthorityState->NavigationGeneration;
				}
				else
				{
		...
		if (Job.Stage == EMovePlanningStage::ProjectCandidates)
		{
			...
			while (AuthorityState->PlanningBudget.CanWork() && Job.Stage == EMovePlanningStage::ProjectCandidates)
			{
					...
					&& Job.NextCandidateProjectionIndex < Job.CandidateProjectionLimit)
				{
					...
					if (Job.InvalidProjectionCache.FindRef(Candidate.CandidateIndex)==AuthorityState->NavigationGeneration) continue;
					const FNavLocation* CachedProjection = Job.ProjectionCache.Find(Candidate.CandidateIndex);
					if (CachedProjection && (Job.NavigationData.Get()!=NavigationData || !NavigationData->IsNodeRefValid(CachedProjection->NodeRef)))
					{ Job.ProjectionCache.Remove(Candidate.CandidateIndex); CachedProjection=nullptr; ++Job.InvalidatedItems; }
					...
					{
						Job.InvalidProjectionCache.Add(Candidate.CandidateIndex,AuthorityState->NavigationGeneration);
				...
				{
					Job.Stage = Job.bExpandingFallback ? EMovePlanningStage::Route : EMovePlanningStage::AssignDestinations;
					Job.bExpandingFallback=false;
					break;
				}
		...
		if (AuthorityState->PlanningBudget.CanWork() && Job.Stage == EMovePlanningStage::AssignDestinations)
		{
			...
			FMoveStageScope Profile(Job.StageProfile[1],AuthorityState->PlanningBudget);
		...
		if (Job.Stage == EMovePlanningStage::Route)
		{
			...
			while (AuthorityState->PlanningBudget.CanWork() && !Job.RouteTasks.IsEmpty())
			{
				...
				if (Task.bPathQueried && Task.Navigation)
				{
					const auto Check=Task.Navigation->Check(*NavigationData,AuthorityState->NavigationGeneration,AuthorityState->PlanningBudget);
					if (Check==FGuLiNavigationDependency::ECheck::Pending) { Job.RouteTasks.Insert(MoveTemp(Task),0); break; }
					if (Check==FGuLiNavigationDependency::ECheck::Invalid)
					{ Task.bPathQueried=false; Task.NextConnector=0; Task.CachedPath.Reset(); ++Job.InvalidatedItems; }
				}
				bool bStartPending=false;
				for (int32 M : Task.MemberPlanIndices)
				{
					auto& Member=Job.Members[M];
					if (Job.FinishedIds.Contains(Member.SoldierId.Value)) continue;
					if (!NavigationData->IsNodeRefValid(Member.CommandStart.NodeRef))
					{
						if (!AuthorityState->PlanningBudget.TakeProjection()) { bStartPending=true; break; }
						FGuLiNavigationWorkBudget::FQueryScope Timer(AuthorityState->PlanningBudget);
						const auto* SI=AuthorityState->SoldierIndexById.Find(Member.SoldierId.Value);
						if (!SI || !ProjectPointToCommanderNavigation(*NavigationSystem,*NavigationData,AuthorityState->Soldiers[*SI].Location,FVector(10,10,5000),Member.CommandStart))
						{ Member.bEligible=false; Member.FailureStage=EGuLiMovePlanFailureStage::StartInvalid; }
					}
				}
				if (bStartPending) { Job.RouteTasks.Insert(MoveTemp(Task),0); break; }
				...
				{
					if (!Job.Members.IsValidIndex(MemberPlanIndex) || Job.FinishedIds.Contains(Job.Members[MemberPlanIndex].SoldierId.Value))
					{
						return true;
					}
					FMoveMemberPlan& Member = Job.Members[MemberPlanIndex];
					const int32* SoldierIndex = AuthorityState->SoldierIndexById.Find(Member.SoldierId.Value);
					if (!Member.bEligible || !SoldierIndex || !AuthorityState->Soldiers.IsValidIndex(*SoldierIndex)
					...
					if (!Task.bPathQueried && Member.CommandStart.Location.Equals(AuthorityState->Soldiers[*SoldierIndex].Location,1.)) { /* retain newly projected reference */ }
					else if (!Task.bPathQueried) Member.CommandStart = AuthorityState->Soldiers[*SoldierIndex].LastValidNavLocation;
				...
				if (Task.bNeedsCandidate)
				{
					...
					{
						auto& Slot=Job.LegalSlots[Task.NextFallbackSlot++]; Job.LastProgressAt=FPlatformTime::Seconds();
						if (Slot.CandidateIndex<=Member.LastCandidateIndex || !IsMoveCandidateAvailableForMember(Job,Member,Slot.CandidateIndex)) continue;
						auto& Projected=Job.ProjectedNavByCandidateIndex.FindChecked(Slot.CandidateIndex);
						if (!NavigationData->IsNodeRefValid(Projected.NodeRef))
						{
							if (!AuthorityState->PlanningBudget.TakeProjection()) { --Task.NextFallbackSlot; break; }
							FGuLiNavigationWorkBudget::FQueryScope Query(AuthorityState->PlanningBudget);
							FNavLocation Updated;
							if (!ProjectPointToCommanderNavigation(*NavigationSystem,*NavigationData,Slot.WorldDestination,FVector(10,10,5000),Updated)
								|| FVector::DistSquared2D(Updated.Location,Slot.WorldDestination)>1.) continue;
							Projected=Updated; Slot.WorldDestination=Updated.Location;
						}
						if (IsMoveCandidateBlockedByHardReservation(Job,Slot.WorldDestination)
							|| !World->GetSubsystem<UGuLiDynamicObstacleRegistrySubsystem>()->GetSnapshot()->IsSegmentClear(Slot.WorldDestination,Slot.WorldDestination,Job.MaximumMemberRadiusCentimeters)) continue;
						ClaimMoveCandidate(Job,M,Slot,Projected);
						Task.bNeedsCandidate=false; break;
					}
					if (Task.bNeedsCandidate)
					{
						if (Task.NextFallbackSlot<Job.LegalSlots.Num()) { Job.RouteTasks.Insert(MoveTemp(Task),0); break; }
						if (RestartMovePlanningWithCompleteCandidatePool(Job)) { Job.RouteTasks.Add(MoveTemp(Task)); break; }
				...
				if (!Task.bPathQueried)
				{
					...
					++Job.Debug.PathQueries; Job.LastProgressAt = FPlatformTime::Seconds();
					{
						...
						Task.bPathValid = BuildCompletePathQuiet(*NavigationSystem, *NavigationData,
							StartMedoid.CommandStart, DestinationMedoid.Destination.WorldDestination, PathPoints, &Task.Navigation, AuthorityState->NavigationGeneration);
					}
					Task.bPathQueried = true; Task.EndAnchor=DestinationMedoid.DestinationNav;
				}
				...
				while (bRouteValid && Task.NextConnector < Task.MemberPlanIndices.Num() * 2)
				{
					...
					auto& Member = Job.Members[Task.MemberPlanIndices[Task.NextConnector / 2]];
					{
						FGuLiNavigationWorkBudget::FQueryScope QueryScope(AuthorityState->PlanningBudget);
						bRouteValid = (Task.NextConnector & 1) == 0
							? HasDirectSurfaceConnection(*NavigationData, Member.CommandStart, PathPoints[0],&Member.StartConnection,AuthorityState->NavigationGeneration)
							: HasDirectSurfaceConnection(*NavigationData, Task.EndAnchor, Member.Destination.WorldDestination,&Member.EndConnection,AuthorityState->NavigationGeneration);
						const auto Snapshot=World->GetSubsystem<UGuLiDynamicObstacleRegistrySubsystem>()->GetSnapshot();
						const bool bStart=(Task.NextConnector & 1)==0;
						const FVector From=bStart ? Member.CommandStart.Location : Task.EndAnchor.Location;
						const FVector To=bStart ? PathPoints[0] : Member.Destination.WorldDestination;
						bRouteValid=bRouteValid && Snapshot->IsSegmentClear(From,To,Job.MaximumMemberRadiusCentimeters,bStart);
						if (bStart) { Member.StartConnectionTarget=To; Member.StartRegion=Snapshot->CaptureRegion(From,To,Job.MaximumMemberRadiusCentimeters); }
						else { Member.EndConnectionOrigin=From; Member.EndRegion=Snapshot->CaptureRegion(From,To,Job.MaximumMemberRadiusCentimeters); }
					}
				...
				Formation.Navigation=MoveTemp(Task.Navigation);
				...
				for (const int32 MemberPlanIndex : Task.MemberPlanIndices)
				{
					...
					Member.EndpointRegion=World->GetSubsystem<UGuLiDynamicObstacleRegistrySubsystem>()->GetSnapshot()->CaptureRegion(Member.DestinationNav.Location,Member.DestinationNav.Location,Job.MaximumMemberRadiusCentimeters);
				...
				Job.DependencyCursor=0; Job.bCommitReady=false; Job.Stage=EMovePlanningStage::ReconcileReservations;
				break; // A ready route becomes a candidate atomic sub-batch immediately.
			}
		...
		{
			FMoveStageScope Profile(Job.StageProfile[3],AuthorityState->PlanningBudget);
			Job.HardReservations.Reset(); Job.HardReservationBuckets.Reset(); Job.ReleasedReservationIds.Reset();
			for (const auto& Member : Job.Members)
				if (Member.bAccepted && !Job.FinishedIds.Contains(Member.SoldierId.Value)) Job.ReleasedReservationIds.Add(Member.SoldierId.Value);
			bool Changed=true;
			while (Changed && AuthorityState->PlanningBudget.CanWork())
			{
				Changed=false;
				for (int32 I=0; I<Job.Members.Num(); ++I)
				{
					auto& Member=Job.Members[I];
					if (!Member.bAccepted || Job.FinishedIds.Contains(Member.SoldierId.Value)) continue;
					if (IsMoveCandidateBlockedByHardReservation(Job,Member.DestinationNav.Location))
					{
						RequeueMoveMember(Job,I,true,false,TEXT("reservation")); Job.ReleasedReservationIds.Remove(Member.SoldierId.Value);
						Changed=true; ++Job.Debug.ReservationConflictCount;
					}
				}
			}
			if (Changed) { Job.Stage=EMovePlanningStage::ReconcileReservations; continue; }
			Job.PreparedFormations.RemoveAll([](const auto& F){ return F.MemberIds.IsEmpty(); });
			Job.DependencyCursor=0;
			Job.bCommitReady=ValidatePreparedMove(Job,*NavigationData,AuthorityState->NavigationGeneration,AuthorityState->PlanningBudget,
				*World->GetSubsystem<UGuLiDynamicObstacleRegistrySubsystem>()->GetSnapshot(),AuthorityState->Soldiers,AuthorityState->SoldierIndexById);
			Job.Stage=(!Job.PreparedFormations.IsEmpty() || Job.RouteTasks.IsEmpty()) ? EMovePlanningStage::ReadyToCommit : EMovePlanningStage::Route;
			if (Job.bCommitReady && !Job.PreparedFormations.IsEmpty())
			{
				auto Trace=GuLiMoveLatency::Context(Job.PlayerState.Get(),Job.SourceCommandId,Job.SharedBatchOrderId);
				Trace.Epoch=Job.AuthorityEpoch; Trace.Execution=Job.Request.ClientCommandId; Trace.Plan=Job.Handle.Value;
				GuLiMoveLatency::Record(TEXT("prepared"),Trace,Job.ReleasedReservationIds.Num());
			}
		}
...
void UGuLiBattleAuthoritySubsystem::CommitReadyMovePlans()
{
	...
	{
		if (!Pointer || !Pointer->bCommitReady || Pointer->Stage==EMovePlanningStage::Completed) continue;
		auto& Job = *Pointer;
		FMoveStageScope Profile(Job.StageProfile[3],AuthorityState->CommitBudget);
		...
		if (Job.NavigationGeneration != AuthorityState->NavigationGeneration)
		{
			...
			Job.bCommitReady=false; Job.DependencyCursor=0; continue;
		}
		...
		for (int32 I = 0; I < Job.Members.Num(); ++I)
		{
			auto& Member = Job.Members[I]; if (!Member.bAccepted || Job.FinishedIds.Contains(Member.SoldierId.Value)) continue;
			const auto* Index = AuthorityState->SoldierIndexById.Find(Member.SoldierId.Value);
			...
			const auto& Soldier = AuthorityState->Soldiers[*Index];
			if (!Soldier.LastValidNavLocation.NodeRef || !Soldier.Location.Equals(Member.CommandStart.Location,1.))
			{
				// Preserve the route and destination; only reconnect from the current start.
				RequeueMoveMember(Job,I,false,true,TEXT("start-moved"));
				Member.CommandStart=Soldier.LastValidNavLocation;
				++Member.CommitConnectionRetryCount;
				Job.ReleasedReservationIds.Remove(Member.SoldierId.Value); bRetry=true;
			}
		...
		if (bRetry)
		{
			Job.bCommitReady=false; Job.Stage=EMovePlanningStage::ReconcileReservations;
			continue;
		}
		int32 AcceptedMembers = 0;
		for (const auto& Member : Job.Members) AcceptedMembers += Member.bAccepted ? 1 : 0;
		if (Job.SourceCommandId)
			if (const auto* Group=AuthorityState->CommandGroups.Find({FObjectKey(Job.PlayerState.Get()),Job.SourceCommandId})) Job.SharedBatchOrderId=Group->Batch;
		if (AcceptedMembers && !Job.SharedBatchOrderId)
		{
			Job.SharedBatchOrderId=AllocateNonZero(AuthorityState->NextBatchOrderId);
			if (Job.SourceCommandId)
			{ auto& Group=AuthorityState->CommandGroups.FindOrAdd({FObjectKey(Job.PlayerState.Get()),Job.SourceCommandId}); Group.Batch=Job.SharedBatchOrderId; Group.LastUsed=FPlatformTime::Seconds(); }
		}
		const uint32 BatchOrderId = Job.SharedBatchOrderId;
		for (const auto& Member : Job.Members) if (Member.bAccepted) AuthorityState->MoveReservations.Remove(Member.SoldierId.Value);
		...
		for (FOrderFormationRuntime& Formation : Job.PreparedFormations)
		{
			...
			for (const FGuLiSoldierId SoldierId : Formation.MemberIds)
			{
				...
				*CommandStart = Soldier.LastValidNavLocation;
				...
				Soldier.ActiveNavigation=Formation.Navigation;
				Soldier.FinalDestinationNavigationGeneration = AuthorityState->NavigationGeneration;
		...
		TSet<int32> Pending;
		for (const auto& Route : Job.RouteTasks) for (int32 I : Route.MemberPlanIndices) Pending.Add(I);
		int32 FinishedNow=0;
		for (auto Receipt : Job.Ack.CohortResults)
		{
			auto* Aggregate=Job.AggregateAck.CohortResults.FindByPredicate([&](const auto& R){ return R.CohortId==Receipt.CohortId; });
			if (!Aggregate) { Receipt.AcceptedMemberMask=0; Aggregate=&Job.AggregateAck.CohortResults.Add_GetRef(Receipt); }
			auto* Updated=Job.UpdatedSelection.Cohorts.FindByPredicate([&](const auto& C){ return C.CohortId==Receipt.CohortId; });
			if (!Updated) { Updated=&Job.UpdatedSelection.Cohorts.AddDefaulted_GetRef(); Updated->CohortId=Receipt.CohortId; }
			Updated->ActiveOrderId=BatchOrderId;
			for (int32 I=0; I<Job.Members.Num(); ++I)
			{
				auto& Member=Job.Members[I];
				if (Member.CohortId!=Receipt.CohortId || Job.FinishedIds.Contains(Member.SoldierId.Value)) continue;
				if (!Member.bAccepted && Pending.Contains(I)) continue;
				if (Member.bAccepted)
				{ Aggregate->AcceptedMemberMask|=1u<<Member.CohortMemberIndex; Updated->MemberIds.AddUnique(Member.SoldierId); }
				else { Job.Progress.Failed.Add(Member.SoldierId); Job.Debug.FailedSoldierIds.AddUnique(Member.SoldierId); }
				Job.FinishedIds.Add(Member.SoldierId.Value); Job.FrozenTaskGenerations.Remove(Member.SoldierId.Value);
				Job.ReleasedReservationIds.Remove(Member.SoldierId.Value); Member.bAccepted=false; ++FinishedNow;
			}
			Aggregate->Result=!Aggregate->AcceptedMemberMask ? EGuLiCommandAckResult::PathFailed
				: Aggregate->AcceptedMemberMask==Aggregate->EligibleMemberMask ? EGuLiCommandAckResult::Accepted : EGuLiCommandAckResult::PartiallyAccepted;
			Updated->AliveCount=uint8(Updated->MemberIds.Num());
		}
		AuthorityState->CommitBudget.Members-=FinishedNow;
		Job.TotalAccepted+=AcceptedMembers;
		if (AcceptedMembers && !Job.FirstCommitAt) Job.FirstCommitAt=FPlatformTime::Seconds();
		Job.PreparedFormations.Reset(); Job.bCommitReady=false;
		Job.Progress.BatchOrderId=BatchOrderId;
		Job.LastProgressAt=FPlatformTime::Seconds();
		auto Trace=GuLiMoveLatency::Context(Job.PlayerState.Get(),Job.SourceCommandId,BatchOrderId);
		Trace.Epoch=Job.AuthorityEpoch; Trace.Execution=Job.Request.ClientCommandId; Trace.Plan=Job.Handle.Value;
		GuLiMoveLatency::Record(TEXT("commit"),Trace,AcceptedMembers);
		if (Pending.IsEmpty())
		{
			for (const auto& Receipt : Job.Ack.CohortResults)
			{
				...
				if (const auto* Aggregate=Job.AggregateAck.CohortResults.FindByPredicate([&](const auto& R){ return R.CohortId==Receipt.CohortId; })) Diagnostic.AcceptedMemberMask=Aggregate->AcceptedMemberMask;
				for (const auto& Member : Job.Members) if (Job.Debug.FailedSoldierIds.Contains(Member.SoldierId)) Diagnostic.FailedSoldierIds.Add(Member.SoldierId);
			}
			Job.Ack.CohortResults.Reset(); Job.Stage=EMovePlanningStage::PrepareBatch;
		}
		else Job.Stage=EMovePlanningStage::Route;
		...
		for (const int32 SoldierIndex : ActiveIndices)
		{
			...
			*MemberPathPointIndex = GuLiCommanderNavigationPolicy::AdvanceMemberPathPointIndex(
			...
			if (bRunsMovementUpdate[SoldierIndex])
			{
				...
				{
					if (!Soldier.bNavigationBudgetPending && !AuthorityState->RecoveryWork.Contains(Soldier.SoldierId.Value)) Soldier.NoProgressSeconds += MovementUpdateDeltaSeconds[SoldierIndex];
	...
	for (const auto& Unit : A.Soldiers)
	{
		...
		if (const auto* E=A.ActiveEndpoints.Find(Unit.SoldierId.Value))
		{
			...
			EndpointMismatches+=!Unit.CanAct() || !Unit.bHasFinalDestination || Unit.ActiveOrderId!=E->ActiveOrderId
				|| !Unit.FinalDestination.Location.Equals(E->FinalDestination,.01) || !Unit.CommandStartLocation.Equals(E->CommandStart,.01);
		}
