...
/** Complete quantized pose in stable world coordinates; the connection codec owns wire encoding. */
USTRUCT()
struct GULISTRIKE_API FGuLiCompressedSoldierPose
{
	...
	// XY: meters. Z: decimeters. Never relative to a moving chunk anchor.
	UPROPERTY(VisibleAnywhere, Category = "Commander|Network")
	...
	// Velocity components in meters/second, not displacement per packet.
	UPROPERTY(VisibleAnywhere, Category = "Commander|Network")
