	...
	const double LocalNowSeconds)
{
	...
	for (int32 SampleIndex = 0; SampleIndex < Chunk.Samples.Num(); ++SampleIndex)
	{
		...
		Sample.Location = CompressedPose.GetWorldLocationCentimeters();
		// Retained diagnostic fields: protocol 9 has no spatial anchor or relative offset.
		Sample.ChunkAnchor = FVector::ZeroVector;
		Sample.RelativeLocation = Sample.Location;
