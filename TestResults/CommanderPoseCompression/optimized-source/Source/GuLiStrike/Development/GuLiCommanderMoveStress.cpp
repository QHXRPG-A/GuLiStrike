...
struct FRun : TSharedFromThis<FRun>
{
	...
	void SamplePoseMetrics(double Now)
	{
		...
		PoseMetricFrames.Last() += FString::Printf(TEXT(",%llu,%llu"), S.SampleGapCount, S.SampleGapTicks);
		for (uint64 Bin : S.SampleGapHistogram) PoseMetricFrames.Last() += FString::Printf(TEXT(",%llu"), Bin);
	}
	...
	bool Tick(float)
	{
		...
		if (CaptureStarted == 0)
		{
			...
			PoseMetricFrames.Add(TEXT("monotonic_seconds,send_ms,receive_ms,ack_ms,capture_ms,wire_read_ms,send_calls,receive_calls,ack_calls,capture_calls,wire_read_calls,decoded_samples,decode_failures,absolute_samples,delta_samples,payload_bytes,cache_bytes,peak_cache_bytes,sample_gap_count,sample_gap_ticks"));
			for (int32 Bin = 1; Bin <= 21; ++Bin) PoseMetricFrames.Last() += FString::Printf(TEXT(",gap_%d"), Bin);
