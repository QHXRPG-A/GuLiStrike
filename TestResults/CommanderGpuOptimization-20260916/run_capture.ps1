...
param(
    [int]$Population = 500,
    [ValidateSet('Unchanged', 'On', 'Off')][string]$SoftwareVrs = 'Unchanged',
...
if ($SoftwareVrs -ne 'Unchanged') {
    $vrsValue = if ($SoftwareVrs -eq 'On') { 1 } else { 0 }
    $cvars += ",r.VRS.ContrastAdaptiveShading $vrsValue,r.VRS.EnableSoftware $vrsValue"
    foreach ($name in @('r.AntiAliasingMethod','r.VolumetricFog','r.Shadow.Virtual.ResolutionLodBiasDirectional','r.Shadow.Virtual.ResolutionLodBiasDirectionalMoving','r.Shadow.Virtual.ResolutionLodBiasLocal','r.Shadow.Virtual.ResolutionLodBiasLocalMoving','r.Lumen.ScreenProbeGather.DownsampleFactor','r.Lumen.ScreenProbeGather.RadianceCache.ProbeResolution','r.LumenScene.DirectLighting.UpdateFactor','r.LumenScene.Radiosity.UpdateFactor','r.Lumen.AsyncCompute','r.VRS.EnableSoftware','r.VRS.ContrastAdaptiveShading')) { $cvars += ",$name" }
}
